class FileUploader {
  ...
  updateFileName() {
    this.nameDisplay.textContent =
      this.input.files.length > 0 ? this.input.files[0].name : this.defaultText;
  }
}
...
class ConferenceRegistration {
    ...
    this.config = Object.assign(
      {
        ...
        radioButtonConfig: [
          {
            ...
            matchValue: "other",
            targetSelector: "#titleOther",
          },
          {
            selector: 'input[name="session"]',
            matchValue: "other_session",
            targetSelector: "#sessionOther",
          },
          {
            selector: 'input[name="course"]',
            matchValue: "other_course",
            targetSelector: "#other_course",
          },
          {
            selector: 'input[name="training"]',
            matchValue: "yes",
            targetSelector: ".has_training",
            isBlock: true,
          },
          {
            selector: 'input[name="payment_form"]',
            matchValue: "advance",
            targetSelector: "#payment-method-section",
            isBlock: true,
          },
        ],
        ...
        fileUploadConfig: [
          {
            input: "#report_file_summary_input",
            name: "#report_file_summary_name",
            defaultText: "FILE BÁO CÁO TÓM TẮT",
          },
          {
            input: "#report_file_full_input",
            name: "#report_file_full_name",
            defaultText: "FILE BÁO CÁO TOÀN VĂN",
          },
          {
            input: "#shortCV_input",
            name: "#shortCV_name",
            defaultText: "SHORT CV",
          },
          {
          ...
          },
          {
            input: "#UNC_input",
            name: "#UNC_name",
            defaultText: "ĐÍNH KÈM SAO KÊ/ UNC NẾU ĐÃ CHUYỂN KHOẢN",
          },
        ],
  ...
  initElements() {
    this.paymentMethods = document.querySelectorAll(
      this.config.paymentMethodSelector
    );
    this.cashPaymentDescription = document.querySelector(
      this.config.cashPaymentDescriptionSelector
    );
    this.wireTransferDescription = document.querySelector(
      this.config.wireTransferDescriptionSelector
    );
  ...
  togglePaymentMethod(target) {
    ...
    this.wireTransferDescription.style.display = isBankTransfer
      ? "block"
      : "none";
    this.cashPaymentDescription.style.display = isBankTransfer
      ? "none"
      : "block";
  }
  ...
  bindRadioToggle(elements, matchValue, targetElement, isBlock = false) {
    elements.forEach((item) => {
      item.addEventListener("click", (e) => {
        if (e.target.checked) {
          targetElement.style.display = isBlock
            ? e.target.value === matchValue
              ? "block"
              : "none"
            : "";
  ...
  bindCheckboxToggle(elements, matchValue, targetElement, isBlock = false) {
    const isMatchValueSelected = () =>
      Array.from(elements).some(
        (checkbox) => checkbox.checked && checkbox.value.includes(matchValue)
      );
  ...
  initFileUploaders() {
    this.fileUploaders = this.config.fileUploadConfig.map(
      (config) =>
