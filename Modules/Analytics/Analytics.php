...
class Analytics
{
  ...
  {
    $response = $this->performQuery($period, "ga:users,ga:pageviews", [
  ...
  {
    $response = $this->performQuery($period, "ga:sessions", [
  ...
  public function fetchTopBrowsers(
    ...
    $topBrowsers = collect($response["rows"] ?? [])->map(function (
      array $browserRow
    ) {
      return [
   ...
   * @return Collection
   */
  protected function summarizeTopBrowsers(
    Collection $topBrowsers,
    int $maxResults
  ): Collection {
