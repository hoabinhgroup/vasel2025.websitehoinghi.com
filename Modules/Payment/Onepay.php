 ...
 class Onepay
{
    ...
    CONST INTERNATIONAL_PAYMENT_VI = 'international_vi';
    CONST INTERNATIONAL_PAYMENT_EN = 'international_en';
    public function __construct()
    {
	    
		$config = config("payment.info");
		
       // $config = require_once(dirname(__FILE__) . '/../Onepay/Config.php');
       
        if (!empty($this->data)) {
	         
            if (array_key_exists('type', $this->data) && $this->data['type'] == self::INTERNATIONAL_PAYMENT) {  # international || domestic
	     
                $this->vpc_Merchant = $config[self::INTERNATIONAL_PAYMENT]['vpc_Merchant'];
                $this->vpc_AccessCode = $config[self::INTERNATIONAL_PAYMENT]['vpc_AccessCode'];
                $this->vpc_SecureHash = $config[self::INTERNATIONAL_PAYMENT]['vpc_SecureHash'];
                $this->vpc_url = $config[self::INTERNATIONAL_PAYMENT]['vpc_url'];
                $params = array(
                    'Title' => $this->data['Title'],
                    'AgainLink' => $this->data['AgainLink'],
                    'vpc_Merchant' => $this->vpc_Merchant,
                    'vpc_AccessCode' => $this->vpc_AccessCode,
                    'vpc_Amount' => $this->data['vpc_Amount'] * 100,
                    'vpc_Command' => 'pay',
                    'vpc_Locale' => 'vn',
                    'vpc_MerchTxnRef' => $this->data['vpc_MerchTxnRef'],
                    'vpc_OrderInfo' => $this->data['vpc_OrderInfo'],
                    'vpc_ReturnURL' => $this->data['vpc_ReturnURL'],
                    'vpc_Version' => '2',
                );
                
                 $this->params = $params;
          
            }else if (array_key_exists('type', $this->data) && $this->data['type'] == self::INTERNATIONAL_PAYMENT_VI) {  # international || domestic
                $this->vpc_Merchant = $config[self::INTERNATIONAL_PAYMENT_VI]['vpc_Merchant'];
                $this->vpc_AccessCode = $config[self::INTERNATIONAL_PAYMENT_VI]['vpc_AccessCode'];
                $this->vpc_SecureHash = $config[self::INTERNATIONAL_PAYMENT_VI]['vpc_SecureHash'];
                $this->vpc_url = $config[self::INTERNATIONAL_PAYMENT_VI]['vpc_url'];
                $params = array(
                    'Title' => $this->data['Title'],
                    'AgainLink' => $this->data['AgainLink'],
                    'vpc_Merchant' => $this->vpc_Merchant,
            ...
            }else if (array_key_exists('type', $this->data) && $this->data['type'] == self::INTERNATIONAL_PAYMENT_EN) {  # international || domestic
                $this->vpc_Merchant = $config[self::INTERNATIONAL_PAYMENT_EN]['vpc_Merchant'];
                $this->vpc_AccessCode = $config[self::INTERNATIONAL_PAYMENT_EN]['vpc_AccessCode'];
                $this->vpc_SecureHash = $config[self::INTERNATIONAL_PAYMENT_EN]['vpc_SecureHash'];
                $this->vpc_url = $config[self::INTERNATIONAL_PAYMENT_EN]['vpc_url'];
                $params = array(
                    'Title' => $this->data['Title'],
                    'AgainLink' => $this->data['AgainLink'],
                    'vpc_Merchant' => $this->vpc_Merchant,
                ...
                $this->vpc_Merchant = $config[self::DOMESTIC_PAYMENT]['vpc_Merchant'];
                $this->vpc_AccessCode = $config[self::DOMESTIC_PAYMENT]['vpc_AccessCode'];
                $this->vpc_SecureHash = $config[self::DOMESTIC_PAYMENT]['vpc_SecureHash'];
                $this->vpc_url = $config[self::DOMESTIC_PAYMENT]['vpc_url'];
                $params = array(
                    'Title' => $this->data['Title'],
                    'vpc_Merchant' => $this->vpc_Merchant,
                    'vpc_AccessCode' => $this->vpc_AccessCode,
                    ...
                    'vpc_OrderInfo' => $this->data['vpc_OrderInfo'],
                    'vpc_ReturnURL' => $this->data['vpc_ReturnURL'],
                    'vpc_TicketNo' => $this->data['vpc_TicketNo'],
                    'vpc_Version' => '2',
                );
            }
    ...
    public function getResponse($response = array())
    {
        ...
        if ($hashValidated == "CORRECT" && $txnResponseCode == "0") {
	        ...
	        $response['result'] = $txnResponseCode;
