...
class MetaBox
{
  ...
  public function addMetaBox(
    ...
    foreach (array_keys($this->metaBoxes[$reference]) as $a_context) {
      foreach (["high", "core", "default", "low"] as $a_priority) {
        if (
          !isset($this->metaBoxes[$reference][$a_context][$a_priority][$id])
        ) {
          continue;
        }
        ...
        if ("core" == $priority) {
          ...
          if (
            false === $this->metaBoxes[$reference][$a_context][$a_priority][$id]
          ) {
            return;
          }
          ...
          if ("default" == $a_priority) {
            $this->metaBoxes[$reference][$a_context]["core"][$id] =
              $this->metaBoxes[$reference][$a_context]["default"][$id];
        ...
        if (empty($priority)) {
          ...
          $title =
            $this->metaBoxes[$reference][$a_context][$a_priority][$id]["title"];
          $callback =
            $this->metaBoxes[$reference][$a_context][$a_priority][$id][
              "callback"
            ];
          $callbackArgs =
            $this->metaBoxes[$reference][$a_context][$a_priority][$id]["args"];
        }
  ...
  public function doMetaBoxes($context, $object = null)
  {
    ...
    if (isset($this->metaBoxes[$reference][$context])) {
      foreach (["high", "sorted", "core", "default", "low"] as $priority) {
        ...
        foreach (
          (array) $this->metaBoxes[$reference][$context][$priority]
          as $box
        ) {
          ...
          $index++;
          $data .= view("base::elements.meta-box-wrap", [
            "box" => $box,
    ...
    echo view("base::elements.meta-box", compact("data", "context"))->render();
  ...
  public function saveMetaBoxData($object, string $key, $value, $options = null)
  {
    try {
      ...
      if (!$fieldMeta) {
        $fieldMeta = app()->make($this->metaBoxRepository->getModel());
      ...
      if (!is_array($value)) {
        ...
        $fieldMeta->meta_value = json_encode([$value]);
      }
