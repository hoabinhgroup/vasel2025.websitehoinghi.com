      ...
      <div class="content-body">
	      ...
	      <div class="container">
	 			@if ($errors->any())
                    ...
                    @endforeach
                @endif
     <form class="form-horizontal" action="{!! route("cmspanel.cate.postEdit", $data['id']) !!}" method="post">
	{{ csrf_field() }}
	...
	<div class="col-md-8">
	 <ul class="nav nav-tabs nav-topline">
		...
		<div class="form-group">
			...
			<input type="" class="form-control" id="" name="link" placeholder="" value="{!! old('link', isset($data)? $data['link']: null) !!}">
		</div>
		<div class="form-group">
		...
		<?php echo ckeditor("description", 350, "100%"); ?>
                      ...
                      <input name="status_vi" type="checkbox" id="switcherySize1" class="switchery" value="1" {!! old('status_vi', (isset($data['status_vi']) && $data['status_vi'] == 1)?' checked': '') !!} />
                      <label for="switcherySize1" class="font-medium-2 ml-1">Bật ngôn ngữ</label>

                    </div>
                        <div class="form-group">
			<label for="">Tiêu đề</label>
			<input type="" class="form-control" id="" name="name_vi" placeholder="" value="{!! old('name_vi', isset($data['name_vi'])? $data['name_vi']: null) !!}">
		</div>
		<div class="form-group">
			<label for="">Đường dẫn</label>
			<input type="" class="form-control" id="" name="link_vi" placeholder="" value="{!! old('link_vi', isset($data['link_vi'])? $data['link_vi']: null) !!}">
		</div>
		<div class="form-group">
			<label for="">Nội dung</label>
			<textarea class="form-control" id="description_vi" name="description_vi" rows="20">{!! old('description_vi', isset($data['description_vi'])? $data['description_vi']: null) !!}</textarea>
		</div>
	...
	<div class="col-md-4">
		<div class="form-group">
			...
			<select class="form-control" name="parent">
				...
				<?php cate_parent($parent, 0, "--", $data["parent"]); ?>
				<option></option>
			</select>
		</div>
		<div class="form-group mt-1">
