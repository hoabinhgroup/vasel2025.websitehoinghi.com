...
use Modules\Setting\Supports\ArrayUtil;
use Exception;
use Form;
  ...
  protected function prepareInsertData(array $data)
  {
    ...
    if ($this->extraColumns) {
      foreach ($data as $key => $value) {
        $dbData[] = array_merge($this->extraColumns, [
          "key" => $key,
          "value" => $value,
        ]);
      }
  ...
  public function render($setting)
  {
    try {
      if (isset($setting["attributes"]["options"]["placeholder"])) {
        $setting["attributes"]["options"]["placeholder"] = trans(
          $setting["attributes"]["options"]["placeholder"]
        );
      }
      if ($this->has($setting["attributes"]["name"])) {
        $setting["attributes"]["value"] = $this->get(
