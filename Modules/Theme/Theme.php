...
class Theme implements ThemeContract
{
    ...
    public function get($theme = null, $collection = false)
    {
        if (is_null($theme) || !$this->has($theme)) {
            return !$collection
                ? $this->themes[$this->activeTheme]->all()
    ...
    public function current($collection = false)
    {
        return !$collection
            ? $this->activeTheme
            : $this->getThemeInfo($this->activeTheme);
    }
    ...
    public function getFullPath($path)
    {
        ...
        if ($this->config["theme.symlink"]) {
            $themePath =
                str_replace(
                    ...
                    "",
                    $this->config["theme.symlink_path"]
                ) .
                DIRECTORY_SEPARATOR .
                $themeName .
                DIRECTORY_SEPARATOR;
        ...
        $assetPath =
            $this->config["theme.folders.assets"] . DIRECTORY_SEPARATOR;
        $fullPath = $themePath . $assetPath . $path;

        if (
            !file_exists($fullPath) &&
            $themeInfo->has("parent") &&
            !empty($themeInfo->get("parent"))
        ) {
            $themePath =
                str_replace(
    ...
    public function lang($fallback)
    {
        ...
        if (count($splitLang) > 1) {
            ...
            if (!$this->lang->has($fallback)) {
                $fallback =
                    $this->getThemeInfo($this->current())->get("parent") .
                    "::" .
                    $splitLang[0];
            }
        }
    ...
    private function scanThemes()
    {
        ...
        foreach ($themeDirectories as $themePath) {
            $themeConfigPath =
                $themePath .
                DIRECTORY_SEPARATOR .
                $this->config["theme.config.name"];
            $themeChangelogPath =
                $themePath .
                DIRECTORY_SEPARATOR .
                $this->config["theme.config.changelog"];

            if (file_exists($themeConfigPath)) {
                $themeConfig = Config::load($themeConfigPath);
                $themeConfig["changelog"] = Config::load(
                    $themeChangelogPath
                )->all();
    ...
    private function loadTheme($theme)
    {
        ...
        $viewPath =
            $themeInfo->get("path") .
            DIRECTORY_SEPARATOR .
            $this->config["theme.folders.views"];
        $langPath =
            $themeInfo->get("path") .
            DIRECTORY_SEPARATOR .
            $this->config["theme.folders.lang"];
