class Louis {
	...
	countCharacter() {
		$.fn.charCounter = function (max, settings) {
			...
			settings = $.extend({
				...
				format: '(%1 ' + LouisVariables.languages.system.character_remain + ')',
				pulse: true,
				delay: 0
			}, settings);
			let p, timeout;
	...
	static initMediaIntegrate() {
		...
		if (jQuery().rvMedia) {
			...
			$(document).on('click', '.btn_remove_image', event =>  {
				event.preventDefault();
				$(event.currentTarget).closest('.image-box').find('.preview-image-wrapper').children('.preview_image').attr('src','/images/default.png');
	...
	handlePortletTools() {
		...
		$('body').on('click', '.portlet > .portlet-title .fullscreen', event =>  {
			...
			if (portlet.hasClass('portlet-fullscreen')) {
				...
				let height = Louis.getViewPort().height -
	...
	static initCodeEditor(id) {
		...
		$('.handle-tool-drag').mousedown(event =>  {
			...
			$('body').attr('data-dragtool', _self.attr('id')).on('mousemove', Louis.onDragTool);
