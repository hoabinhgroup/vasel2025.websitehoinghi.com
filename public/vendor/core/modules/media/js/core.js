class Louis {
  ...
  static blockUI(options) {
    ...
    if (options.animate) {
      ...
      html =
        '<div class="loading-message ' +
        (options.boxed ? "loading-message-boxed" : "") +
        '"><img src="/vendor/core/images/loading-spinner-blue.gif" alt="loading"></div>';
      ...
      html =
        '<div class="loading-message ' +
        (options.boxed ? "loading-message-boxed" : "") +
        '"><img src="/vendor/core/images/loading-spinner-blue.gif" alt="loading"><span>&nbsp;&nbsp;' +
        (options.message ? options.message : "LOADING...") +
        "</span></div>";
    }
  ...
  static showNotice(messageType, message) {
    ...
    switch (messageType) {
      case "error":
        messageHeader = LouisVariables.languages.notices_msg.error;
        break;
      case "success":
        messageHeader = LouisVariables.languages.notices_msg.success;
        break;
    }
  ...
  static handleError(data) {
    if (typeof data.errors !== "undefined" && !_.isArray(data.errors)) {
      Louis.handleValidationError(data.errors);
    } else {
      if (typeof data.responseJSON !== "undefined") {
        if (typeof data.responseJSON.errors !== "undefined") {
          if (data.status === 422) {
            Louis.handleValidationError(data.responseJSON.errors);
          }
        } else if (typeof data.responseJSON.message !== "undefined") {
          Louis.showError(data.responseJSON.message);
        } else {
          $.each(data.responseJSON, (index, el) => {
            $.each(el, (key, item) => {
              Louis.showError(item);
            });
          });
        }
      } else {
        Louis.showError(data.statusText);
      }
    }
  }
      ...
      settings = $.extend(
        {
          ...
          format:
            "(%1 " + LouisVariables.languages.system.character_remain + ")",
          pulse: true,
          delay: 0,
        },
        settings
  ...
  static initResources() {
    ...
    Louis.initDatePicker(".datepicker");
    if (jQuery().mCustomScrollbar) {
      Louis.callScroll($(".list-item-checkbox"));
    }
  ...
  static initMediaIntegrate() {
    if (jQuery().louisMedia) {
      $('[data-type="louis-media-standard-alone-button"]').louisMedia({
        multiple: false,
          ...
          onSelectFiles: (files, $el) => {
            ...
            switch ($el.data("action")) {
              case "media-insert-ckeditor":
                $.each(files, (index, file) => {
                  ...
                  if (file.type === "youtube") {
                    ...
                    CKEDITOR.instances[$el.data("result")].insertHtml(
              ...
              case "attachment":
                ...
                console.log("el", $el.closest(".attachment-wrapper"));
  ...
  static initCodeEditor(id) {
    ...
    $(".handle-tool-drag").mousedown((event) => {
      let _self = $(event.currentTarget);
      _self
        .attr("data-start_h", _self.parent().find(".CodeMirror").height())
        .attr("data-start_y", event.pageY);
      $("body")
        .attr("data-dragtool", _self.attr("id"))
        .on("mousemove", Louis.onDragTool);
      $(window).on("mouseup", Louis.onReleaseTool);
    });
  }
  ...
  static onReleaseTool() {
    $("body").off("mousemove", Louis.onDragTool);
    $(window).off("mouseup", Louis.onReleaseTool);
  }
